...
export const submissionService = {
  ...
  async getByTypePaged(type: string, page: number = 1, limit: number = 50, status: string = 'ALL', search: string = '') {
    ...
    if (user) {
      // Determine which scope to use based on type
      let scope = null;
      const hrTypes = ['Libur Mandiri', 'Lembur', 'Izin', 'Cuti Tahunan', 'Cuti Melahirkan', 'Cuti'];
      ...
      if (scope?.mode === 'limited') {
        const allowedIds = scope.location_ids || [];
        if (allowedIds.length > 0) {
          query = query.in('account.location_id', allowedIds);
        } else {
          // If limited but no IDs, return empty
  ...
  async getPendingCounts() {
    ...
    if (user) {
      ...
      if (hrScope?.mode === 'limited') {
        const allowedIds = hrScope.location_ids || [];
        if (allowedIds.length > 0) {
          attendanceQuery = attendanceQuery.in('account.location_id', allowedIds);
        } else {
          // If limited but no IDs, return the results fetched so far
          return counts;
        }
      }
    }
