...
import { PendingSubmissionsProvider, usePendingSubmissions } from './context/PendingSubmissionsContext';
import { supabase } from './lib/supabase';
import { AuthUser } from './types';

// Helper component to handle mobile menu content with context access
...
}> = ({ user, activeTab, setActiveTab, setIsMobileMenuOpen, unreadReimbursements, unreadCompensations, unreadDispensations }) => {
  ...
  return (
    <>
      ...
      <div className="mt-4">
        ...
        <NavItemMobile id="leave" icon={Plane} label="Libur Mandiri" indent badge={(user.is_hr_admin || user.role === 'admin') ? pendingSubmissions['Libur Mandiri'] : 0} />
        <NavItemMobile id="overtime" icon={Timer} label="Presensi Lembur" indent badge={(user.is_hr_admin || user.role === 'admin') ? pendingSubmissions['Lembur'] : 0} />
        <NavItemMobile id="permission" icon={ClipboardList} label="Izin" indent badge={(user.is_hr_admin || user.role === 'admin') ? pendingSubmissions['Izin'] : 0} />
        <NavItemMobile id="annual_leave" icon={Calendar} label="Cuti Tahunan" indent badge={(user.is_hr_admin || user.role === 'admin') ? pendingSubmissions['Cuti Tahunan'] : 0} />
        {(user?.gender === 'Perempuan' || user?.role === 'admin' || user?.is_hr_admin) && (
          <NavItemMobile id="maternity_leave" icon={Heart} label="Cuti Melahirkan" indent badge={(user.is_hr_admin || user.role === 'admin') ? pendingSubmissions['Cuti Melahirkan'] : 0} />
        )}
        {isAdmin && (
...
const App: React.FC = () => {
  ...
  if (isMobile) {
    return (
      <PendingSubmissionsProvider user={user}>
